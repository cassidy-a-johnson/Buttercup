...
#10x
 if test -n "$(find ./genomic_data/10x/ -maxdepth 0 -empty)"
then
...
#Pacbio Hifi
 ...
 then
    ...
    echo "/sbatch --partition=vgl --exclude=node[141-165]  --nice /rugpfs/fs0/vgl/store/cjohnson02/bin/Merqury_QV_slurm/_submit_meryl2_build_fastq.sh 21 R.fofn summary_hifi vgl | awk '{print $4}' >> hifi_meryl.jid"
           sbatch --partition=vgl --exclude=node[141-165]  --nice /rugpfs/fs0/vgl/store/cjohnson02/bin/Merqury_QV_slurm/_submit_meryl2_build_fastq.sh 21 R.fofn summary_hifi vgl | awk '{print $4}' >> hifi_meryl.jid
fi  
