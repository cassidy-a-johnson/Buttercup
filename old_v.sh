...
sbatch --partition=vgl --wait --job-name=aws_10x --output=%x.out aws s3 cp --no-sign-request --recursive --exclude '*' --include "*R1*.fastq.gz" --include "*R2*.fastq.gz" s3://genomeark/species/Pangasianodon_hypophthalmus/fPanHyp1/genomic_data/10x/ ./genomic_data/10x | awk '{printf $4 ", "}' >> 10x.id
#illumina:
echo "/
...
sbatch --partition=vgl --wait --job-name=aws_pacbiohifi --output=%x.out aws s3 cp --no-sign-request --recursive --exclude '*' --include "*.fastq.gz" --include "*.fastq" s3://genomeark/species/Pangasianodon_hypophthalmus/fPanHyp1/genomic_data/pacbio_hifi/ ./genomic_data/pacbio_hifi | awk '{printf $4}' >> hifi.id
